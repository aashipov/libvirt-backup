...
# Sums actual-size of all VM disks, compares to available at df "${CURRENT_BACKUP_DIR}"
#
# VM_NAMES_TO_BACK_UP is set in the .env
# CURRENT_BACKUP_DIR is set in the environment
# ------------------------------------------------------------
...
clean_obsolete_backups() {
    ...
    then
        find "${ANOTHER_SERVER_ANOTHER_BACKUP_DIR}/" -type d -depth -mindepth 1 -mtime "${DAYS_TO_KEEP_BACKUPS}" -exec rm -rf {} \;
    fi

    if [ -d "${BACKUP_DIR}" ]
    then
        find "${BACKUP_DIR}/" -type d -depth -mindepth 1 -mtime "${DAYS_TO_KEEP_BACKUPS}" -exec rm -rf {} \;
    fi
...
push_backups_to_another_server() {
    ...
    rsync --times --partial --recursive --delete --rsh="ssh -o BatchMode=yes" "${BACKUP_DIR}/" "${ANOTHER_SERVER_IP}:${ANOTHER_SERVER_ANOTHER_BACKUP_DIR}/" || die "Push failed"
